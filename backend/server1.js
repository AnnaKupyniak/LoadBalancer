...
app.post('/solve', async (req, res) => {
  ...
  const serverName = "Сервер 1"; // змінюється на Сервер 2 для другого сервера
...
app.get('/history', async (req, res) => {
  try {
    ...
    const tasks = await Task.find({ server: "Сервер 1" })
                            .sort({ createdAt: -1 })
                            .limit(20);
    res.json(tasks);
  } catch (err) {
...
app.listen(8001, () => console.log('🌐 Backend running on http://localhost:8001'));
