...
const runningTasks = {}; // для відстеження таймерів активних задач
const taskQueue = [];    // черга очікуючих задач
let runningCount = 0;
const MAX_CONCURRENT_TASKS = 2;

// --- Функція запуску задачі з BigInt ---
...
app.get('/history', async (req, res) => {
  const tasks = await Task.find({ server: "Сервер 1" }).sort({ createdAt: -1 }).limit(20);
  res.json(tasks);
});
