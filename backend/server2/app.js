...
async function startWorkerTask(workerParams, task) {
  ...
  return new Promise((resolve, reject) => {
    ...
    worker.on('message', (msg) => {
      if (msg.type === 'progress') {
        const stepData = {
          server: serverName,
          step: msg.stepInfo,
          result: msg.currentValue.length > 50 
            ? msg.currentValue.substring(0, 50) + '...' 
            : msg.currentValue
        };
        
        saveQueue.push({ type: 'progress', ...msg, stepData });
        processSaveQueue(taskId, task, saveQueue);
        
      } else if (msg.type === 'done') {
        saveQueue.push({ type: 'done', ...msg });
        ...
        processSaveQueue(taskId, task, saveQueue).then(async () => {
          if (task.type === 'part' && task.parentTaskId) {
            try {
              ...
              if (!actualCoordinatorUrl) {
                console.error(`[${SERVER_ID}] ❌ ПОМИЛКА: Не знайдено coordinatorUrl для ${taskId}`);
          ...
          console.error(`Помилка збереження результату для ${taskId}:`, err);
          delete runningTasks[taskId];
          runningCount = Math.max(0, runningCount - 1);
          reject(err);
        });
    ...
    worker.on('error', (err) => {
      console.error(`Worker error for ${taskId}:`, err);
      saveQueue.push({
        type: 'error',
        stepData: { server: 'System', step: 'Worker Error', result: err.message }
      });
    ...
    worker.on('exit', (code) => {
      if (code !== 0 && code !== 1 && runningTasks[taskId]) {
        console.error(`Worker ${taskId} stopped unexpectedly with exit code ${code}`);
        delete runningTasks[taskId];
        runningCount = Math.max(0, runningCount - 1);
      }
    });
